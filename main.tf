...
module "aci_external_endpoint_group" {
  ...
  for_each                    = { for epg in local.external_endpoint_groups : epg.key => epg.value if try(local.modules.aci_external_endpoint_group, true) }
  tenant                      = local.tenant.name
...
module "aci_filter" {
  ...
  for_each    = { for filter in try(local.tenant.filters, []) : filter.name => filter if try(local.modules.aci_filter, true) }
  tenant      = local.tenant.name
...
module "aci_contract" {
  ...
  for_each    = { for contract in try(local.tenant.contracts, []) : contract.name => contract if try(local.modules.aci_contract, true) }
  tenant      = local.tenant.name
  name        = "${each.value.name}${local.defaults.apic.tenants.contracts.name_suffix}"
  alias       = try(each.value.alias, "")
  description = try(each.value.description, "")
  scope       = try(each.value.scope, local.defaults.apic.tenants.contracts.scope)
  qos_class   = try(each.value.qos_class, local.defaults.apic.tenants.contracts.qos_class)
  target_dscp = try(each.value.target_dscp, local.defaults.apic.tenants.contracts.target_dscp)
  subjects = [for subject in try(each.value.subjects, []) : {
...
module "aci_oob_contract" {
  ...
  for_each    = { for contract in try(local.tenant.oob_contracts, []) : contract.name => contract if local.tenant.name == "mgmt" && try(local.modules.aci_oob_contract, true) }
  name        = "${each.value.name}${local.defaults.apic.tenants.oob_contracts.name_suffix}"
  alias       = try(each.value.alias, "")
  description = try(each.value.description, "")
  scope       = try(each.value.scope, local.defaults.apic.tenants.oob_contracts.scope)
  subjects = [for subject in try(each.value.subjects, []) : {
...
module "aci_imported_contract" {
  ...
  for_each        = { for contract in try(local.tenant.imported_contracts, []) : contract.name => contract if try(local.modules.aci_imported_contract, true) }
  tenant          = local.tenant.name
...
module "aci_ospf_interface_policy" {
  ...
  for_each                = { for policy in try(local.tenant.policies.ospf_interface_policies, []) : policy.name => policy if try(local.modules.aci_ospf_interface_policy, true) }
  tenant                  = local.tenant.name
  name                    = "${each.value.name}${local.defaults.apic.tenants.policies.ospf_interface_policies.name_suffix}"
  description             = try(each.value.description, "")
  cost                    = try(each.value.cost, local.defaults.apic.tenants.policies.ospf_interface_policies.cost)
  dead_interval           = try(each.value.dead_interval, local.defaults.apic.tenants.policies.ospf_interface_policies.dead_interval)
  hello_interval          = try(each.value.hello_interval, local.defaults.apic.tenants.policies.ospf_interface_policies.hello_interval)
  network_type            = try(each.value.network_type, local.defaults.apic.tenants.policies.ospf_interface_policies.network_type)
  priority                = try(each.value.priority, local.defaults.apic.tenants.policies.ospf_interface_policies.priority)
  lsa_retransmit_interval = try(each.value.lsa_retransmit_interval, local.defaults.apic.tenants.policies.ospf_interface_policies.lsa_retransmit_interval)
  lsa_transmit_delay      = try(each.value.lsa_transmit_delay, local.defaults.apic.tenants.policies.ospf_interface_policies.lsa_transmit_delay)
  passive_interface       = try(each.value.passive_interface, local.defaults.apic.tenants.policies.ospf_interface_policies.passive_interface)
  mtu_ignore              = try(each.value.mtu_ignore, local.defaults.apic.tenants.policies.ospf_interface_policies.mtu_ignore)
  advertise_subnet        = try(each.value.advertise_subnet, local.defaults.apic.tenants.policies.ospf_interface_policies.advertise_subnet)
...
module "aci_bgp_timer_policy" {
  ...
  for_each                = { for pol in try(local.tenant.policies.bgp_timer_policies, []) : pol.name => pol if try(local.modules.aci_bgp_timer_policy, true) }
  tenant                  = local.tenant.name
  name                    = "${each.value.name}${local.defaults.apic.tenants.policies.bgp_timer_policies.name_suffix}"
  description             = try(each.value.description, "")
  graceful_restart_helper = try(each.value.graceful_restart_helper, local.defaults.apic.tenants.policies.bgp_timer_policies.graceful_restart_helper)
  hold_interval           = try(each.value.hold_interval, local.defaults.apic.tenants.policies.bgp_timer_policies.hold_interval)
  keepalive_interval      = try(each.value.keepalive_interval, local.defaults.apic.tenants.policies.bgp_timer_policies.keepalive_interval)
  maximum_as_limit        = try(each.value.maximum_as_limit, local.defaults.apic.tenants.policies.bgp_timer_policies.maximum_as_limit)
...
module "aci_bgp_address_family_context_policy" {
  ...
  for_each               = { for pol in try(local.tenant.policies.bgp_address_family_context_policies, []) : pol.name => pol if try(local.modules.aci_bgp_address_family_context_policy, true) }
  tenant                 = local.tenant.name
  name                   = "${each.value.name}${local.defaults.apic.tenants.policies.bgp_address_family_context_policies.name_suffix}"
  description            = try(each.value.description, "")
  enable_host_route_leak = try(each.value.enable_host_route_leak, local.defaults.apic.tenants.policies.bgp_address_family_context_policies.enable_host_route_leak)
  ebgp_distance          = try(each.value.ebgp_distance, local.defaults.apic.tenants.policies.bgp_address_family_context_policies.ebgp_distance)
  ibgp_distance          = try(each.value.ibgp_distance, local.defaults.apic.tenants.policies.bgp_address_family_context_policies.ibgp_distance)
  local_distance         = try(each.value.local_distance, local.defaults.apic.tenants.policies.bgp_address_family_context_policies.local_distance)
  local_max_ecmp         = try(each.value.local_max_ecmp, 0)
  ebgp_max_ecmp          = try(each.value.ebgp_max_ecmp, local.defaults.apic.tenants.policies.bgp_address_family_context_policies.ebgp_max_ecmp)
...
module "aci_dhcp_relay_policy" {
  ...
  for_each    = { for policy in try(local.tenant.policies.dhcp_relay_policies, []) : policy.name => policy if try(local.modules.aci_dhcp_relay_policy, true) }
  tenant      = local.tenant.name
  name        = "${each.value.name}${local.defaults.apic.tenants.policies.dhcp_relay_policies.name_suffix}"
  description = try(each.value.description, "")
  providers_ = [for provider in try(each.value.providers, []) : {
    ip                      = provider.ip
    type                    = provider.type
    tenant                  = try(provider.tenant, local.tenant.name)
...
module "aci_dhcp_option_policy" {
  ...
  for_each    = { for policy in try(local.tenant.policies.dhcp_option_policies, []) : policy.name => policy if try(local.modules.aci_dhcp_option_policy, true) }
  tenant      = local.tenant.name
...
module "aci_route_control_route_map" {
  ...
  for_each    = { for rm in try(local.tenant.policies.route_control_route_maps, []) : rm.name => rm if try(local.modules.aci_route_control_route_map, true) }
  tenant      = local.tenant.name
  name        = "${each.value.name}${local.defaults.apic.tenants.policies.route_control_route_maps.name_suffix}"
  description = try(each.value.description, "")
  contexts = [for ctx in try(each.value.contexts, []) : {
...
module "aci_ip_sla_policy" {
  ...
  for_each    = { for policy in try(local.tenant.policies.ip_sla_policies, []) : policy.name => policy if try(local.modules.aci_ip_sla_policy, true) }
  tenant      = local.tenant.name
  name        = "${each.value.name}${local.defaults.apic.tenants.policies.ip_sla_policies.name_suffix}"
  description = try(each.value.description, "")
  multiplier  = try(each.value.multiplier, local.defaults.apic.tenants.policies.ip_sla_policies.multiplier)
  frequency   = try(each.value.frequency, local.defaults.apic.tenants.policies.ip_sla_policies.frequency)
  sla_type    = try(each.value.sla_type, local.defaults.apic.tenants.policies.ip_sla_policies.sla_type)
...
module "aci_match_rule" {
  ...
  for_each    = { for rule in try(local.tenant.policies.match_rules, []) : rule.name => rule if try(local.modules.aci_match_rule, true) }
  tenant      = local.tenant.name
  name        = "${each.value.name}${local.defaults.apic.tenants.policies.match_rules.name_suffix}"
  description = try(each.value.description, "")
  regex_community_terms = [for regex in try(each.value.regex_community_terms, []) : {
  ...
  community_terms = [for comm in try(each.value.community_terms, []) : {
  ...
  prefixes = [for prefix in try(each.value.prefixes, []) : {
    ip          = prefix.ip
...
module "aci_set_rule" {
  ...
  for_each                    = { for rule in try(local.tenant.policies.set_rules, []) : rule.name => rule if try(local.modules.aci_set_rule, true) }
  tenant                      = local.tenant.name
  name                        = "${each.value.name}${local.defaults.apic.tenants.policies.set_rules.name_suffix}"
  description                 = try(each.value.description, "")
  community                   = try(each.value.community, "")
  community_mode              = try(each.value.community_mode, local.defaults.apic.tenants.policies.set_rules.community_mode)
  tag                         = try(each.value.tag, null)
  dampening                   = try(each.value.dampening.half_life, null) != null || try(each.value.dampening.max_suppress_time, null) != null || try(each.value.dampening.reuse_limit, null) != null || try(each.value.dampening.suppress_limit, null) != null
  dampening_half_life         = try(each.value.dampening.half_life, local.defaults.apic.tenants.policies.set_rules.dampening.half_life)
  dampening_max_suppress_time = try(each.value.dampening.max_suppress_time, local.defaults.apic.tenants.policies.set_rules.dampening.max_suppress_time)
  dampening_reuse_limit       = try(each.value.dampening.reuse_limit, local.defaults.apic.tenants.policies.set_rules.dampening.reuse_limit)
  dampening_suppress_limit    = try(each.value.dampening.suppress_limit, local.defaults.apic.tenants.policies.set_rules.dampening.suppress_limit)
  weight                      = try(each.value.weight, null)
  next_hop                    = try(each.value.next_hop, "")
  metric                      = try(each.value.metric, null)
  preference                  = try(each.value.preference, null)
  metric_type                 = try(each.value.metric_type, "")
  additional_communities = [
    for comm in try(each.value.additional_communities, []) : {
      community   = comm.community
      description = try(comm.description, "")
    }
  ]
  set_as_path          = try(each.value.set_as_path.criteria, null) != null || try(each.value.set_as_path.count, null) != null || try(each.value.set_as_path.order, null) != null
  set_as_path_criteria = try(each.value.set_as_path.criteria, local.defaults.apic.tenants.policies.set_rules.set_as_path.criteria)
  set_as_path_count    = try(each.value.set_as_path.count, local.defaults.apic.tenants.policies.set_rules.set_as_path.count)
  set_as_path_order    = try(each.value.set_as_path.order, local.defaults.apic.tenants.policies.set_rules.set_as_path.order)
  set_as_path_asn      = try(each.value.set_as_path.asn, null)
  next_hop_propagation = try(each.value.next_hop_propagation, local.defaults.apic.tenants.policies.set_rules.next_hop_propagation)
...
module "aci_bfd_interface_policy" {
  ...
  for_each                  = { for pol in try(local.tenant.policies.bfd_interface_policies, []) : pol.name => pol if try(local.modules.aci_bfd_interface_policy, true) }
  tenant                    = local.tenant.name
  name                      = "${each.value.name}${local.defaults.apic.tenants.policies.bfd_interface_policies.name_suffix}"
  description               = try(each.value.description, "")
  subinterface_optimization = try(each.value.subinterface_optimization, local.defaults.apic.tenants.policies.bfd_interface_policies.subinterface_optimization)
  detection_multiplier      = try(each.value.detection_multiplier, local.defaults.apic.tenants.policies.bfd_interface_policies.detection_multiplier)
  echo_admin_state          = try(each.value.echo_admin_state, local.defaults.apic.tenants.policies.bfd_interface_policies.echo_admin_state)
  echo_rx_interval          = try(each.value.echo_rx_interval, local.defaults.apic.tenants.policies.bfd_interface_policies.echo_rx_interval)
  min_rx_interval           = try(each.value.min_rx_interval, local.defaults.apic.tenants.policies.bfd_interface_policies.min_rx_interval)
...
module "aci_qos_policy" {
  ...
  for_each    = { for pol in try(local.tenant.policies.qos, []) : pol.name => pol if try(local.modules.aci_qos_policy, true) }
  tenant      = local.tenant.name
  name        = "${each.value.name}${local.defaults.apic.tenants.policies.qos.name_suffix}"
  description = try(each.value.description, "")
  alias       = try(each.value.alias, "")
  dscp_priority_maps = [
    for map in try(each.value.dscp_priority_maps, []) : {
  ...
  dot1p_classifiers = [
    for c in try(each.value.dot1p_classifiers, []) : {
...
module "aci_pim_policy" {
  ...
  strict_rfc                   = try(each.value.strict_rfc, local.defaults.apic.tenants.policies.pim_policies.strict_rfc)
  designated_router_delay      = try(each.value.designated_router_delay, local.defaults.apic.tenants.policies.pim_policies.designated_router_delay)
...
module "aci_l4l7_device" {
  ...
  for_each         = { for device in local.l4l7_devices : device.name => device if try(local.modules.aci_l4l7_device, true) }
  tenant           = local.tenant.name
  name             = each.value.name
  alias            = each.value.alias
...
module "aci_redirect_policy" {
  ...
  for_each               = { for policy in try(local.tenant.services.redirect_policies, []) : policy.name => policy if try(local.modules.aci_redirect_policy, true) }
  tenant                 = local.tenant.name
  name                   = "${each.value.name}${local.defaults.apic.tenants.services.redirect_policies.name_suffix}"
  alias                  = try(each.value.alias, "")
  description            = try(each.value.description, "")
  anycast                = try(each.value.anycast, local.defaults.apic.tenants.services.redirect_policies.anycast)
  type                   = try(each.value.type, local.defaults.apic.tenants.services.redirect_policies.type)
  hashing                = try(each.value.hashing, local.defaults.apic.tenants.services.redirect_policies.hashing)
  threshold              = try(each.value.threshold, local.defaults.apic.tenants.services.redirect_policies.threshold)
  max_threshold          = try(each.value.max_threshold, local.defaults.apic.tenants.services.redirect_policies.max_threshold)
  min_threshold          = try(each.value.min_threshold, local.defaults.apic.tenants.services.redirect_policies.min_threshold)
  pod_aware              = try(each.value.pod_aware, local.defaults.apic.tenants.services.redirect_policies.pod_aware)
  resilient_hashing      = try(each.value.resilient_hashing, local.defaults.apic.tenants.services.redirect_policies.resilient_hashing)
  threshold_down_action  = try(each.value.threshold_down_action, local.defaults.apic.tenants.services.redirect_policies.threshold_down_action)
  ip_sla_policy          = try("${each.value.ip_sla_policy}${local.defaults.apic.tenants.policies.ip_sla_policies.name_suffix}", "")
  redirect_backup_policy = try("${each.value.redirect_backup_policy}${local.defaults.apic.tenants.services.redirect_backup_policies.name_suffix}", "")
...
module "aci_redirect_backup_policy" {
  ...
  for_each    = { for policy in try(local.tenant.services.redirect_backup_policies, []) : policy.name => policy if try(local.modules.aci_redirect_backup_policy, true) }
  tenant      = local.tenant.name
...
module "aci_redirect_health_group" {
  ...
  for_each    = { for health_group in try(local.tenant.services.redirect_health_groups, []) : health_group.name => health_group if try(local.modules.aci_redirect_health_group, true) }
  tenant      = local.tenant.name
...
module "aci_service_graph_template" {
  ...
  for_each            = { for sg_template in try(local.tenant.services.service_graph_templates, []) : sg_template.name => sg_template if try(local.modules.aci_service_graph_template, true) }
  tenant              = local.tenant.name
  name                = "${each.value.name}${local.defaults.apic.tenants.services.service_graph_templates.name_suffix}"
  description         = try(each.value.description, "")
  alias               = try(each.value.alias, "")
  template_type       = try(each.value.template_type, local.defaults.apic.tenants.services.service_graph_templates.template_type)
  redirect            = try(each.value.redirect, local.defaults.apic.tenants.services.service_graph_templates.redirect)
  share_encapsulation = try(each.value.share_encapsulation, local.defaults.apic.tenants.services.service_graph_templates.share_encapsulation)
  device_name         = "${each.value.device.name}${local.defaults.apic.tenants.services.l4l7_devices.name_suffix}"
  device_tenant       = try(each.value.device.tenant, local.tenant.name)
  device_function     = length(local.l4l7_devices) != 0 ? [for device in local.l4l7_devices : try(device.function, []) if device.name == each.value.device.name][0] : "None"
  device_copy         = (length(local.l4l7_devices) != 0 ? [for device in local.l4l7_devices : try(device.copy_device, []) if device.name == each.value.device.name][0] : false)
...
module "aci_device_selection_policy" {
  ...
  for_each                                                = { for pol in try(local.tenant.services.device_selection_policies, []) : "${pol.contract}/${pol.service_graph_template}" => pol if try(local.modules.aci_device_selection_policy, true) }
  tenant                                                  = local.tenant.name
...
module "aci_tenant_span_destination_group" {
  ...
  for_each                        = { for span in try(local.tenant.policies.span.destination_groups, []) : span.name => span if try(local.modules.aci_tenant_span_destination_group, true) }
  tenant                          = local.tenant.name
...
module "aci_tenant_span_source_group" {
  ...
  for_each    = { for span in try(local.tenant.policies.span.source_groups, []) : span.name => span if try(local.modules.aci_tenant_span_source_group, true) }
  tenant      = local.tenant.name
